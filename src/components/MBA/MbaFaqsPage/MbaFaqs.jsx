...
const MbaFaqs = () => {
  ...
  return (
    <div className='mba_faq mba_Container' id='faq'>
        {/* This perticular div was responsible for all the content in the MBA faq's page */}
    <div className="mba_wrapper-3 ">
    {/* This perticular div was responsible only for heading tag in the MBA faq's page */}
    <div className='mba_faqheading' >
        <h1>FREQUENTLY ASKED QUESTIONS</h1>
    </div>
    {/* Here we are using mapping function to get the data from mba_Footer_Data which is there in the same file */}
        <div className="mba_accordion2">
            {mba_Footer_Data.map((item, i2) => (
                // This perticular div was responsible for question toggle and answer toggle
                <div className="item2">
                    {/* This perticular div was responsible only for question toggle  */}
                    <div className="mba_faqtitle" onClick={() => toggle2(i2)}>
                        <h2>{item.Qustion}</h2>
                        <span>{selected === i2 ? '-' : '+'}</span>
                    </div>
                    {/* This perticular div was responsible only for answer toggle */}
                    <div className={selected === i2 ? 'mba_content-show2' : 'pg_content2'}>{item.answer}</div>
                </div>
            ))}
...
export default MbaFaqs

// Here we are using all the data regarding faq's 
const mba_Footer_Data = [
    {
